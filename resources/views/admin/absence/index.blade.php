...
@section('content')
    ...
    @endif
    <div class="fixed z-50 inset-0 m-6 overflow-y-auto hidden" aria-labelledby="modal-title" role="dialog"
    ...
    <div class="flex mx-2">
        ...
        <div class="w-1/2 md:w-1/3">
            <label for="filter" class="block text-gray-700 font-bold mb-2">Filtrer par année :</label>
            <div class="relative">
                <select id="filter_year" name="filter"
                    class="block appearance-none w-full bg-white border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline">
                    @foreach ($years as $yr)
                        <option {{ $date->format('Y') == $yr ? 'selected' : '' }} value="{{ $yr }}">
                            {{ $yr }}</option>
                    @endforeach
                </select>
                <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700">
                    <svg class="fill-current h-4 w-4" viewBox="0 0 20 20">
                        <path d="M9 11l3-3 3 3M9 16l3-3 3 3M9 6l3 3 3-3"></path>
                    </svg>
                </div>
            </div>
        </div>
        <div class="w-1/2  md:w-1/3">
            <label for="filter" class="block text-gray-700 font-bold mb-2">Filtrer par mois :</label>
            <div class="relative">
                <select id="filter_month" name="filter"
                    class="block appearance-none w-full bg-white border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline">
                    @foreach ($months as $key => $month)
                        <option {{ $date->format('m') == $key + 1 ? 'selected' : '' }} value={{ $month }}>
                            {{ $month }}</option>
                    @endforeach
    ...
    <section id="scroll-content" class="m-2 overflow-x-auto"
        style="width: 98%;border-radius: 10px;box-shadow:2px 2px 10px orange; max-height: 450px;border: 2px black solid">
        ...
        <table class="table-auto w-full">
            ...
            <tbody class="text-xs font-semibold uppercase text-gray-400 bg-gray-50">
                @foreach ($employees as $employee)
                    <tr>
                        <td class="sticky left-0 z-10 bg-gray-500 text-white px-6 py-4">
                            {{ $employee->user->getFirstName() }} {{ $employee->user->getLastName() }}</td>
                        @for ($i = 1; $i <= $daysInCurrentMonth; $i++)
                            <th
                                class="{{ $i == now()->day && now()->format('Y-m-d') === $date->format('Y-m-d') ? 'bg-sky-200' : '' }} {{ $i > \Carbon\Carbon::today()->day && \Carbon\Carbon::parse(now()->year . '-' . now()->month . '-' . $i)->format('l') != 'Sunday' && now()->format('Y-m-d') === $date->format('Y-m-d') ? 'bg-sky-400' : '' }}
                                ...
                                @else
                                    ...
                                    @else
                                        ...
                                        @elseif($employee->isAbsentInThisDay($current_year . '-' . $current_month . '-' . $i))
                                            <div
                                                class="w-6 h-6 rounded-full flex justify-center items-center bg-red-500 text-white">
                                                <svg class="w-4 h-4 fill-current" viewBox="0 0 20 20">
                                                    <path
                                                        d="M10 0a10 10 0 1 0 10 10A10 10 0 0 0 10 0zm5.7 12.3l-.7.7L10 10.7l-4.3 4.3-.7-.7L9.3 10 4.7 5.4l.7-.7L10 9.3l4.3-4.3.7.7L10.7 10l4.6 4.6z" />
                                                </svg>
                                            </div>
                                        @else
                                            <div
                                                class="w-6 h-6 rounded-full flex justify-center items-center bg-green-500 text-white">
                                                <svg class="w-4 h-4 fill-current" viewBox="0 0 20 20">
...
@section('script')
    <script>
        ...
        $(Filter_month).change(e => {
            if (months[current_date.getMonth() == Filter_month.val() && current_date.getYear() == Filter_year
                    .val()])
                window.location.href = `http://localhost:8000/admin/absence`
            else
                window.location.href =
                `http://localhost:8000/admin/absence?year=${Filter_year.val()}&month=${Filter_month.val()}`
        })
